...
function getResponsiveColumnCount(columns: number): number {
  const normalizedColumns = Math.max(1, Math.floor(columns));

  if (typeof window === 'undefined') return normalizedColumns;
...
export const PhotoGrid: React.FC<PhotoGridProps> = ({ images, columns, onImageClick }) => {
  ...
  const [columnCount, setColumnCount] = useState(() => getResponsiveColumnCount(columns));
  const videoRefs = useRef<Map<number, HTMLVideoElement>>(new Map());

  useEffect(() => {
    const updateColumnCount = () => {
      setColumnCount(getResponsiveColumnCount(columns));
    };
  ...
  const imageColumns = useMemo(() => {
    const nextColumns: Array<Array<{ image: PhotoItem; index: number }>> = Array.from(
      { length: columnCount },
      () => [],
    );

    images.forEach((image, index) => {
      nextColumns[index % columnCount].push({ image, index });
    });
  ...
  return (
    <div 
      ...
      style={{
        '--photowall-columns': columnCount,
      } as React.CSSProperties}
    >
      {imageColumns.map((columnImages, columnIndex) => (
        <div key={columnIndex} className="photowall-column">
